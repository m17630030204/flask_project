...
def login():
    ...
    if not user.check_password(passport):
        return jsonify(errno=RET.PWDERR,errmsg="用户户名或密码错误")
    # 4.保存用户登录状态
    ...
    # 但是其实可以不用自己去写 db.session.commit()，前提是对SQLAlchemy有个相关配置
    # try:
    #     db.session.commit()
    # except Exception as  e:
    #     db.session.rollback()
...
def register():
    ...
    if not real_sms_code:
        return jsonify(errno = RET.NODATA, errmsg= "验证马不正确")
