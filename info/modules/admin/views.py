...
def news_edit_detail():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            render_template('admin/news_edit_detail.html', errmsg="查询数据错误")

        if not news:
            render_template('admin/news_edit_detail.html', errmsg="为查询到数据")
        ...
        for category in categories:
            #取到分类的字典
            cate_dict = category.to_dict()
            #判断当前遍历到分类是否是当前新闻的分类 如果是则添加is_selected为True
...
def user_count():
    ...
    #取到今天的时间字符串
    today_data_str = ('%d-%02d-%02d' %(t.tm_year,t.tm_mon,t.tm_mday))
    #转成时间对象
    today_data = datetime.strptime(today_data_str,"%Y-%m-%d")

    for i in range(0,31):
        #取代某一天的0,点0分
        begin_date = today_data - timedelta(days=i)
        #取到下一天的0点0分
        end_date = today_data - timedelta(days=(i-1))
